...
export async function POST(request: NextRequest) {
  try {
    ...
    if (!conversationId || !paymentId) {
      // Parametreler eksik - ödeme sayfasına error ile yönlendir (303 = GET redirect)
      return NextResponse.redirect(
        `${baseUrl}/checkout/odeme?payment=error&reason=missing_params`,
        { status: 303 }
      );
    }

    if (!isSuccess) {
      // 3D Secure başarısız - ödeme sayfasına error ile yönlendir (303 = GET redirect)
      return NextResponse.redirect(
        `${baseUrl}/checkout/odeme?payment=error&reason=3ds_failed&mdStatus=${mdStatus}`,
        { status: 303 }
      );
    }

    // Ödeme başarılı - onay sayfasına success ile yönlendir (303 = GET redirect)
    return NextResponse.redirect(
      `${baseUrl}/checkout/onay?payment=success&order=${conversationId}`,
      { status: 303 }
    );
    ...
    const baseUrl = process.env.NEXT_PUBLIC_SITE_URL || 
      `${request.nextUrl.protocol}//${request.nextUrl.host}`;
    // Hata durumunda ödeme sayfasına error ile yönlendir (303 = GET redirect)
    return NextResponse.redirect(
      `${baseUrl}/checkout/odeme?payment=error&reason=server_error`,
      { status: 303 }
    );
  }
}
